  ...
  handlerConf = (function() {
    ...
    handlerConf.prototype.reload = function() {
      ...
      return this.lmConf.getConf().then(function(conf) {
        ...
        return console.log("Can't get configuration", e);
      });
    };
    ...
    handlerConf.prototype.substitute = function(expr) {
      return expr.replace(/\seq\s/g, ' === ').replace(/\sne\s/g, ' !== ').replace(/\sor\s/g, ' || ').replace(/\sand\s/g, ' && ').replace(/\$date\b/g, 'this.date()').replace(/\$vhost\b/g, 'this.hostname()').replace(/\$ip\b/g, 'this.remote_ip()').replace(/\$(_*[a-zA-Z]\w*)/g, 'session.$1');
    };
