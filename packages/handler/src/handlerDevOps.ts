...
class LemonldapNGHandlerDevOps extends LemonldapNGHandler {
  ...
  grant( req: express.Request | http.IncomingMessage, uri: string, session: LLNG_Session ) {
    ...
    if ( this.tsv.defaultCondition[vhost] && (Date.now()/1000 - this.tsv.lastVhostUpdate[vhost] < 600 ) ) {
      // @ts-ignore
      return <Promise<boolean>>super.grant(req, uri, session);
    }
    // @ts-ignore: cgiParams is tested
    let base = req.cgiParams && req.cgiParams.RULES_URL
      // @ts-ignore: same
      ? req.cgiParams.RULES_URL
      : ( this.tsv.loopBackUrl || 'http://127.0.0.1' ) + '/rules.json';
    if (!/^(https?):\/\/([^\/:]+)(?::(\d+))?(.*)$/.test(base)) {
      return new Promise<boolean>( (resolve, reject) => reject(`Bad loopBackUrl ${base}`) );
    }
    ...
    return new Promise<boolean>( (resolve,reject) => {
      this.loadVhostConfig( req, vhost, lvOpts ).then( () => {
        this.grant( req, uri, session ).then( res => {
          resolve(res);
        }).catch( e => reject(e) );
      }).catch( err => {
        console.error(err);
  ...
  loadVhostConfig(req: express.Request | http.IncomingMessage, vhost: string, lvOpts: Vhost_Config) {
    return new Promise<boolean>( (resolve,reject) => {
      ...
      let req = http.request( lvOpts, (resp: http.ServerResponse) => {
        ...
        resp.on('end', () => {
          ...
          if( str === '' ) {
            this.tsv.defaultCondition[vhost] = () => {return true};
          ...
          try {
            ...
            if (vhostConfig.headers && typeof vhostConfig.headers === 'object') {
              let sub = Object.keys(vhostConfig.headers).map( name => {
                this.tsv.headerList[vhost].push(name);
                return `'${name}': ${this.substitute(vhostConfig.headers[name])}`
              }).join(',');
