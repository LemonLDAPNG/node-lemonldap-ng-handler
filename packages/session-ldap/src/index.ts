...
class LDAPSession implements Session_Accessor{
  ...
  constructor(args: SessionLDAP_Args) {
    ['ldapServer','ldapConfBase'].forEach( (k) => {
      if(!args[<keyof SessionLDAP_Args>k]) throw new Error(`Missing ${k} argument`)
    });
    const ldapServer = args.ldapServer.match(/^ldap/)
      ? args.ldapServer
      : `ldap://${args.ldapServer}`;
    const ldapCa = args.ldapServer.match(/^ldaps/)
      ? args.ldapCAFile || ''
      : '';
  ...
  update(data: LLNG_Session) {
    return new Promise<boolean>( (resolve, reject) => {
      ...
      this.ldap.modify(`${this.idAttr}=${data._session_id},${this.base}`, change, err => {
        if(err) return reject(err);
        resolve(true);
      });
    });
