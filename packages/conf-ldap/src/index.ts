  ...
  available() {
    return new Promise<number[]>( (resolve, reject) => {
      this.client.bind( this.dn, this.pwd, (err) => {
        ...
        this.client.search(this.base, opt, (err: Error|null, res: SearchCallbackResponse) => {
  ...
  load(cfgNum: number, fields: string[] = []) {
    return new Promise<LLNG_Conf>( (resolve, reject) => {
      this.client.bind(this.dn, this.pwd, err => {
        ...
        this.client.search(`${this.idAttr}=lmConf-${cfgNum},${this.base}`, opt, (err: Error|null, res: SearchCallbackResponse) => {
